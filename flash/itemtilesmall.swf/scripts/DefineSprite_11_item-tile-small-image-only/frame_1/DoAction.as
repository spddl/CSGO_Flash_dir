...
function SetTestItemInfo(SlotIndex, ItemIndex, ImagePath, ItemName, Rarity)
{
   ...
   LoadItemImage(ImagePath,90,67);
   DisplayTileText(SeperateName(ItemName));
   SetColor(Rarity,this.RarityColor);
...
function GetItemData(TileDesc)
{
   ...
   switch(TileDesc)
   {
      case "Loadout":
         ...
         SetColor(GetRarityColor(),this.RarityColor);
      ...
      case "Acknowledge":
         ...
         break;
      case "RareItem":
         SetColor("#ffd700",this.RarityColor);
         SetSprayPaintStyles(false);
         SetStickers(GetStickerCount());
         CTDot._visible = false;
         TDot._visible = false;
         FlairDot._visible = false;
...
function SetSprayPaintStyles(bLarge)
{
   SpraysBg._visible = false;
   SpraysTintLayer._visible = false;
   this.ItemImage.blendMode = "normal";
   var _loc3_ = IsSpraySealed();
   var _loc4_ = IsSprayPaint();
   if(_loc3_ || _loc4_)
   {
      var _loc2_ = GetSprayTintColor();
      if(_loc2_ != "" && _loc2_ != null)
      {
         SetColor(GetSprayTintColor(),this.SpraysTintLayer);
         if(bLarge)
         {
            LoadItemImage(GetImagePath(),180,135,true);
         }
         else
         {
            LoadItemImage(GetImagePath(),90,67,true);
         }
         this.ItemImage.blendMode = "multiply";
         SpraysTintLayer._visible = true;
      }
      SpraysBg._visible = true;
   }
}
...
function IsSpraySealed()
{
   return _global.CScaleformComponent_Inventory.DoesItemMatchDefinitionByName(this._ItemXuid,this._ItemID,"spray");
}
function IsSprayPaint()
{
   return _global.CScaleformComponent_Inventory.DoesItemMatchDefinitionByName(this._ItemXuid,this._ItemID,"spraypaint");
}
function GetSprayTintColor()
{
   return _global.CScaleformComponent_Inventory.GetSprayTintColorCode(this._ItemID);
}
...
function LoadItemImage(ImagePath, numWidth, numHeight, bIsTinitedSpray)
{
   ...
   if(!bIsTinitedSpray)
   {
      _loc1_.loadClip(_loc3_,ItemImage.DynamicItemImage.Image);
   }
   else
   {
      _loc1_.loadClip(_loc3_,SpraysTintLayer);
   }
}
...
function SetEquippedDot(IsEquippedCt, IsEquippedT, IsNoTeam, strTeam)
{
   ...
   if(IsNoTeam == true && (GetSlotID() == "flair0" || GetSlotID() == "musickit" || GetSlotID() == "spray0"))
   {
